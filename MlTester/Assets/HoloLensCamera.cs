        ...
        public double Exposure
        {
            ...
            set
            {
                ...
                if (ManualExposureSupported)
                {
                    ...
                    double val = UnityEngine.Mathf.Clamp((float)exponent, (float)min, (float)max);

                    val = (double)((int)((val - min) / step) * step + min);

                    bool worked = videoDeviceController.Exposure.TrySetValue(val);
                    UnityEngine.Debug.Log($"Setting exposure worked = {worked}");
                }
                else
                {
...
#pragma warning restore 0067
        ...
        public async void Start(StreamDescription streamDesc)
        {
            ...
            if (mediaCapture != null)
            {
                ...
                {
                    // TODO: handle case where we couldn't get the format we asked for
                }
        ...
        public async void Stop()
        {
            ...
            lock (stateLock)
            {
                if (State == CameraState.CapturingContinuous || State == CameraState.CapturingSingle || State == CameraState.Ready || State == CameraState.Starting)
                {
        ...
        private void OnMediaFrameArrived(MediaFrameReader sender, MediaFrameArrivedEventArgs args)
        {
            ...
            lock (stateLock)
            {
                issueCallback = State == CameraState.CapturingContinuous || State == CameraState.CapturingSingle;
            ...
            if (issueCallback)// || (State == CameraState.Ready && CaptureMode == CaptureMode.SingleLowLatency))
            {
            ...
            if (frame != null)
            {
                ...
                if (issueCallback && OnFrameCaptured != null && numCallbacks > 0)
                {
                    /// TODO: add refs
